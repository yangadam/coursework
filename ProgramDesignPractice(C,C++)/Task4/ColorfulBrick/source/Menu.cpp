...
Menu::MenuResult Menu::show(sf::RenderWindow& window, const char* fileName)
{
	...
	window.draw(sprite);
	if (!strcmp(fileName, "media/images/settings.png"))
	{
		select(window);
	}
	window.display();
...
void Menu::select(sf::RenderWindow& rw)
{
	...
	case 16:
		{
			sf::Texture texture;
			texture.loadFromFile("media/images/golden.png");
	...
	case 14:
		{
			sf::Texture texture;
			texture.loadFromFile("media/images/lightblue.png");
			sf::Sprite choice(texture);
	...
	case 12:
		{
			sf::Texture texture;
			texture.loadFromFile("media/images/lightgreen.png");
			sf::Sprite choice(texture);
