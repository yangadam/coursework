...
int BrickManager::handleClick(Position& pos)
{
	...
	{
		ServiceLocator::GetAudio()->PlaySound("audio/speedmatch1.wav");
	}
	else if(score == 300)
	{
		score += 100;
		ServiceLocator::GetAudio()->PlaySound("audio/speedmatch5.wav");
	}
	else if (score == 400)
	{
		score += 200;
		ServiceLocator::GetAudio()->PlaySound("audio/speedmatch.wav");
	}
	return score;
}
